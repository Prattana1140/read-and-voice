...
type PaymentStatus = "pending" | "paid" | "failed" | "cancelled" | "all";
...
async function loadPayments() {
  ...
  try {
    ...
    errorMessage.value = error?.response?.data?.message || "โหลดรายการอนุมัติไม่สำเร็จ";
  } finally {
    loading.value = false;
  }
}
...
async function updatePayment(item: PaymentItem, status: "paid" | "failed" | "cancelled") {
  ...
  try {
    ...
    message.value =
      data?.message ||
      (status === "paid" ? "อนุมัติรายการสำเร็จ" : "อัปเดตรายการสำเร็จ");
    await loadPayments();
  } catch (error: any) {
    errorMessage.value = error?.response?.data?.message || "อัปเดตรายการไม่สำเร็จ";
  } finally {
    savingKey.value = "";
  }
}
...
function typeLabel(type: PaymentType) {
  ...
  if (type === "order") return "คำสั่งซื้อ";
  ...
  <main class="approval-page">
    ...
    <section class="toolbar">
      <label>
        ...
        <div class="mobile-tabs" aria-label="กรองสถานะรายการอนุมัติ">
          <button
