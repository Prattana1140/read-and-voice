...
const login = async () => {
  ...
  try {
    await loginWithSocialProvider(router, "facebook");
  } catch (err: any) {
    error.value = err.response?.data?.message || "เข้าสู่ระบบด้วย Facebook ไม่สำเร็จ";
  } finally {
    loading.value = false;
  }
};
...
<template>
  <main class="social-page">
    <section class="card">
      <h1>เข้าสู่ระบบด้วย Facebook</h1>
      <p>เชื่อมบัญชี Facebook เพื่อเข้าใช้งาน Read and Voice</p>
